...
public class GUIHandler extends JFrame implements ActionListener {
	...
	public GUIHandler(int difficulty) throws Exception {
		...
		String scoreLabel = String.format("<html><font size=+2><b>SCORE: %d</b></font></html>",
				session.getScore());
			...
			if (questionAnswered) {
				try {
					if (!session.isItTheEnd()) {
						...
						answerFrame.SetMovies(movies[0], movies[1], movies[2],
								movies[3]);
	...
	public void SetAnswer(String _answer) {
		...
		try {
			if (session.evaluate(answer)) {
				answerStatus.setText(
						"<html><font size =+1>Your answer is <font color=green>CORRECT</font></html>");

				session.addToScore(100);
				String scoreText = String.format(
						"<html><font size=+2><b>SCORE: %d</b></font></html>",
						session.getScore());
				score.setText(scoreText);
			} else {
				answerStatus.setText(
						"<html><font size =+1>Your answer is <font color=red>INCORRECT</font></html>");
